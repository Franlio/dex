...
module.exports = async function (deployer, _network, accounts) {
  // accounts -> an array of Ethereum addresses generated by Ganache
  ...
  const seedTokenBalance = async (token, trader) => {
    // faucet of ERC20 tokens only working in development
    // mainnet doesn't have faucet
    await token.faucet(trader, amount)
    // The trader has some amount of ERC20 tiokens
    // approve the decentralize exchange to spend these tokens on the behave of the trader
    await token.approve(
      dex.address,
      amount,
      { from: trader }
    ...
    // Deposit ERC20 tokens from the trader to the smart contract of the smart contract exchange
    await dex.deposit(
      amount,
      // Convert to byte32
  ...
  // Create fake trades
  // Create limit order from trader1
  await dex.createLimitOrder(BAT, 1000, 10, SIDE.BUY, { from: trader1 });
  // Create market order from trader2 to match the limit order created before
  await dex.createMarketOrder(BAT, 1000, SIDE.SELL, { from: trader2 });
  // Increase the time of the blockchain by one second
  // If you want to visualize the trades in the front-end,
  // the trades will take place at different time which simulate the real market condition
  ...
  // Create fake orders
  // Create the limit orders for different tokens simultaneously with Promise.all
  await Promise.all([
