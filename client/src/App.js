...
function App({ web3, accounts, contracts }) {
  ...
  // Fetch the orders from the smart contract
  ...
  const listenToTrades = token => {
    ...
    // Reset setTrades array
    setTrades([]);
    // The current connection to the blockchain
    ...
    // Store to the state to cancel the previous connection
    setListener(listener);
  }

  // Will be called everytime when the token was changed by the user
  const selectToken = token => {
    setUser({ ...user, selectedToken: token });
  }

  // Send the tokens from the wallet to the smart contract
  const deposit = async amount => {
    // Approve the dex smart contract to send the tokens
  ...
  // When the component mount into the DOM at the first time
  useEffect(() => {
    const init = async () => {
  ...
  // When you change the token selection,
  // refetch the order for the new token
  useEffect(() => {
    const init = async () => {
      const [balances, orders] = await Promise.all([
        getBalances(
    ...
    // If there's a token currently selected
